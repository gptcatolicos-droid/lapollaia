...
function DashboardPage(){
  ...
  React.useEffect(()=>{
    if(!user) return
    api(`/api/results/user/${user.id}`).then(data=>{
      if(data&&typeof data==='object'&&!Array.isArray(data)){
        setStats({total:data.total||0,played:data.played||0,position:data.position||null})
      } else {
        // Legacy array response fallback
        const arr=Array.isArray(data)?data:[]
        const total=arr.reduce((s,r)=>(r.points_earned||0)+(r.extra_pts||0)+s,0)
        setStats({total,played:arr.length,position:null})
      }
