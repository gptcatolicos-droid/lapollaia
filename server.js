...
async function setMatchTeams(db, match, team1, team2, changes, opts={}){
  if(!match) return
  const allowPartial=!!opts.allowPartial
  if(!allowPartial&&(!team1||!team2)) return
  const next1=team1||null, next2=team2||null
...
async function syncKnockoutBracket(db=pool){
  ...
  const byMatchNum=new Map(matches.map(m=>[m.match_num,m]))
  for(let i=0;i<r16.length;i++){
    const [homeFrom,awayFrom]=R16_FROM_R32[i]||[]
    await setMatchTeams(db,r16[i],resultWinnerTeam(byMatchNum.get(homeFrom)),resultWinnerTeam(byMatchNum.get(awayFrom)),changes,{allowPartial:true})
  }

  const quarters=byPhase('quarters')
  for(let i=0;i<quarters.length;i++)
    await setMatchTeams(db,quarters[i],resultWinnerTeam(r16[i*2]),resultWinnerTeam(r16[i*2+1]),changes,{allowPartial:true})

  const semis=byPhase('semis')
  for(let i=0;i<semis.length;i++)
    await setMatchTeams(db,semis[i],resultWinnerTeam(quarters[i*2]),resultWinnerTeam(quarters[i*2+1]),changes,{allowPartial:true})
  ...
  if(third) await setMatchTeams(db,third,semiLoser(semis[0]),semiLoser(semis[1]),changes,{allowPartial:true})
  if(final) await setMatchTeams(db,final,resultWinnerTeam(semis[0]),resultWinnerTeam(semis[1]),changes,{allowPartial:true})
...
app.get('/api/results/user/:userId', auth, async(req,res)=>{
  try{
    const tid=req.user.tournamentId
    // Get avatar for this user in this tournament
    const {rows:[av]}=await pool.query('SELECT id FROM avatars WHERE user_id=$1 AND tournament_id=$2 AND is_active=TRUE',[req.params.userId,tid])
    if(!av) return res.json({total:0,played:0,position:null})

    // Get prediction points
    const {rows:preds}=await pool.query(`
      SELECT p.match_id, p.score_home, p.score_away, p.points_earned
      FROM predictions p
      WHERE p.avatar_id=$1
      ORDER BY p.match_id`,[av.id])

    // Get extra_predictions points
    const {rows:extras}=await pool.query('SELECT SUM(points_earned) as pts FROM extra_predictions WHERE avatar_id=$1',[av.id])

    // Get registration bonus
    const {rows:regBonus}=await pool.query('SELECT points FROM registration_bonus WHERE avatar_id=$1',[av.id])

    // Get trivia points
    const {rows:trivia}=await pool.query('SELECT SUM(points_earned) as pts FROM trivia_answers WHERE avatar_id=$1 AND is_correct=TRUE',[av.id])

    // Get adjustment
    const {rows:adjRow}=await pool.query('SELECT points_adjustment FROM avatars WHERE id=$1',[av.id])

    const predPts=preds.reduce((s,r)=>s+(r.points_earned||0),0)
    const extraPts=+(extras[0]?.pts||0)
    const regPts=+(regBonus[0]?.points||0)
    const trivPts=+(trivia[0]?.pts||0)
    const adjPts=+(adjRow[0]?.points_adjustment||0)
    const total=predPts+extraPts+regPts+trivPts+adjPts
    const played=preds.filter(p=>p.score_home!=null).length

    // Get position from ranking
    const {rows:ranking}=await pool.query(`
      SELECT av2.id FROM avatars av2
      LEFT JOIN (SELECT avatar_id, SUM(points_earned) as pts FROM predictions GROUP BY avatar_id) pp ON pp.avatar_id=av2.id
      LEFT JOIN (SELECT avatar_id, SUM(points_earned) as pts FROM extra_predictions GROUP BY avatar_id) ep ON ep.avatar_id=av2.id
      LEFT JOIN special_predictions sp ON sp.avatar_id=av2.id
      LEFT JOIN registration_bonus rb ON rb.avatar_id=av2.id
      LEFT JOIN (SELECT avatar_id, SUM(points_earned) as pts FROM trivia_answers WHERE is_correct=TRUE GROUP BY avatar_id) ta ON ta.avatar_id=av2.id
      WHERE av2.tournament_id=$1 AND av2.is_active=TRUE
      ORDER BY (COALESCE(pp.pts,0)+COALESCE(ep.pts,0)+
        COALESCE(sp.champion_pts,0)+COALESCE(sp.surprise_pts,0)+COALESCE(sp.balon_pts,0)+COALESCE(sp.guante_pts,0)+COALESCE(sp.bota_pts,0)+
        COALESCE(rb.points,0)+COALESCE(ta.pts,0)+COALESCE(av2.points_adjustment,0)) DESC`,[tid])
    const position=ranking.findIndex(r=>r.id===av.id)+1

    res.json({total,played,position:position||null,predictions:preds})
  }catch(e){console.error('results/user:',e);res.json({total:0,played:0,position:null})}
})
...
app.post('/api/admin/results', auth, tournamentAdmin, async(req,res)=>{
  ...
  try{
    ...
    // Recalculate extra_predictions points for this match
    const {rows:extras}=await pool.query(`
      SELECT ep.* FROM extra_predictions ep
      JOIN avatars av ON av.id=ep.avatar_id WHERE ep.match_id=$1 AND av.tournament_id=$2`,[matchId,req.user.tournamentId])
    for(const ep of extras){
      const epts=calcExtraPoints(ep,result)
      await pool.query('UPDATE extra_predictions SET points_earned=$1 WHERE id=$2',[epts,ep.id])
    }
