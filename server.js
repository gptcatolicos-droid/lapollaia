...
app.post('/api/bracket/suggest', auth, async(req,res)=>{
  ...
  try{
    const msg = await anthropic.messages.create({
      ...
      system:`Eres un experto analista de fútbol mundial. Genera un bracket completo y CREÍBLE del Mundial FIFA 2026.

GRUPOS OFICIALES FIFA 2026 (48 equipos, 12 grupos de 4):
- Grupo A: Alemania, Arabia Saudita, Argelia, Argentina
- Grupo B: Australia, Austria, Bélgica, Brasil
- Grupo C: Cabo Verde, Canadá, Qatar, Colombia
- Grupo D: Corea del Sur, Costa de Marfil, Croacia, Curazao
- Grupo E: Ecuador, Egipto, Escocia, España
- Grupo F: EE.UU., Francia, Ghana, Haití
- Grupo G: Inglaterra, Irak, Japón, Jordania
- Grupo H: Marruecos, México, Noruega, Nueva Zelanda
- Grupo I: Países Bajos, Panamá, Paraguay, Portugal
- Grupo J: Senegal, Sudáfrica, Suiza, Túnez
- Grupo K: Uruguay, Irán, Uzbekistán, Bosnia
- Grupo L: República Checa, Turquía, Francia... (ver abajo)

RANKING FIFA APROXIMADO (para determinar resultados realistas):
Top 5: Francia(2), Argentina(3), España(4), Inglaterra(5), Brasil(6), Portugal(8), Bélgica(3), Países Bajos(7), Alemania(16), Uruguay(17), Colombia(18), Marruecos(14), Japón(17), México(15), EE.UU.(13), Croacia(10).
Equipos débiles (fuera del top 40): Qatar, Curazao, Haití, Jordania, Nueva Zelanda, Cabo Verde, Irak, Uzbekistán, Bosnia, Panamá, Sudáfrica, Arabia Saudita, Argelia, Egipto, Ghana, Túnez.

LÓGICA CRÍTICA QUE DEBES SEGUIR:
1. CADA equipo aparece MÁXIMO UNA VEZ en cada ronda.
2. Los ganadores del Round of 32 son quienes juegan en Round of 16, y así sucesivamente.
3. Equipos débiles (Qatar, Curazao, Haití, Jordania, Nueva Zelanda, etc.) NUNCA llegan a semis o final.
4. Los equipos top del ranking (Francia, Argentina, España, Brasil, Inglaterra, Portugal, Alemania, Países Bajos) son los que razonablemente llegan a cuartos y semis.
5. Las semifinales deben ser SIEMPRE equipos top, no sorpresas improbables.
6. Los marcadores deben ser realistas: diferencias de 1-2 goles en partidos parejos, no 5-0 en semis.
7. El campeón elegido debe ganar su semifinal y la final de manera creíble.
8. El Round of 32 tiene 32 partidos (matchs 1 a 32 en el array). El Round of 16 tiene 16. Quarters tiene 8. Semis tiene 4.

ROUND OF 32 - CRUCES OFICIALES FIFA 2026 (ejemplos de matchups típicos):
- 1er Grupo A vs 2do Grupo B, 1er Grupo C vs 2do Grupo D, etc.
- Los primeros de grupo fuertes (Argentina 1A, Brasil 1B, España 1E) deben pasar el R32 sin problemas.

Responde SOLO en JSON válido sin markdown, sin comentarios, con esta estructura:
{
  "round32": [{"match":1,"home":"EQUIPO","away":"EQUIPO","winner":"EQUIPO","home_score":N,"away_score":N}, ... exactamente 32 objetos],
  "round16": [{"match":1,"home":"EQUIPO","away":"EQUIPO","winner":"EQUIPO","home_score":N,"away_score":N}, ... exactamente 16 objetos],
  "quarters": [{"match":1,"home":"EQUIPO","away":"EQUIPO","winner":"EQUIPO","home_score":N,"away_score":N}, ... exactamente 8 objetos],
  "semis": [{"match":1,"home":"EQUIPO","away":"EQUIPO","winner":"EQUIPO","home_score":N,"away_score":N}, ... exactamente 4 objetos],
  "third": {"home":"EQUIPO","away":"EQUIPO","winner":"EQUIPO","home_score":N,"away_score":N},
  "final": {"home":"EQUIPO","away":"EQUIPO","winner":"CAMPEON","home_score":N,"away_score":N},
  "champion": "CAMPEON"
}
IMPORTANTE: Usa los nombres en español exactamente como están en los grupos arriba (ej: "Argentina", "Brasil", "España", "Francia", "Alemania", "Inglaterra", "Países Bajos", "Portugal", "Uruguay", "Colombia", "Marruecos", "EE.UU.", "Japón", "México", "Croacia").`,
      messages:[{role:'user',content:`Genera el bracket completo y CREÍBLE del Mundial 2026 donde ${champion} es el campeón.

REGLAS OBLIGATORIAS:
- Las 4 semifinalistas DEBEN ser equipos del top 15 del ranking FIFA (Francia, Argentina, España, Brasil, Inglaterra, Portugal, Alemania, Países Bajos, Colombia, Uruguay, Bélgica, Marruecos, México, Croacia, EE.UU.).
- ${champion} llega a la final y la gana. Elige un rival finalista también del top 5 (no Canadá, no Escocia, no Qatar).
- Los ganadores de cada ronda deben ser los mismos equipos que aparecen en la ronda siguiente (no puedes tener un equipo en semis que no ganó en cuartos).
- Verifica antes de responder: ¿aparece el mismo equipo dos veces en la misma ronda? Si sí, corrígelo.
- Los equipos débiles (Qatar, Curazao, Haití, Nueva Zelanda, Jordania, Irak, Ghana, etc.) salen en Round of 32 o Round of 16 como máximo.`}]
    })
    const text = msg.content[0].text.trim().replace(/```json|```/g,'').trim()
